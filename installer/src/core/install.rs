...
mod tests {
    ...
    fn rejects_zip_slip_paths() {
        ...
        assert!(safe_zip_entry_name("/evil.dll").is_none());
        assert!(safe_zip_entry_name("BepInEx/plugins/GuildrunAccess/GuildrunAccess.dll").is_some());
    }
    ...
    fn fresh_install_then_uninstall_leaves_game_dir_clean() {
        let dir = tempfile::tempdir().unwrap();
        fs::write(dir.path().join("Guildrun.exe"), "game").unwrap();
        let zip_path = dir.path().join("release.zip");
        create_zip(
            &zip_path,
            &[
                (paths::PLUGIN_REL, "plugin"),
                ("BepInEx/plugins/GuildrunAccess/lang/en.txt", "strings"),
                ("BepInEx/plugins/GuildrunAccess/prism.dll", "speech"),
                ("winhttp.dll", "loader"),
            ],
        );
        ...
        assert!(dir.path().join("BepInEx/plugins/GuildrunAccess/prism.dll").exists());
        assert!(matches!(
            classify_install(dir.path()),
        ...
        assert!(dir.path().join("Guildrun.exe").exists());
    }
    ...
    fn upgrade_prunes_files_the_new_zip_no_longer_ships() {
        ...
        create_zip(
            ...
            &[
                (paths::PLUGIN_REL, "plugin"),
                ("BepInEx/plugins/GuildrunAccess/GuildrunAccess.Module.dll", "module"),
            ],
        );
        let m1 = install_from_zip(
            &zip1,
        ...
        assert!(!dir.path().join("BepInEx/plugins/GuildrunAccess/GuildrunAccess.Module.dll").exists());
        assert_eq!(
    ...
    fn test_asset() -> Asset {
        Asset {
            name: "GuildrunAccess-v1.2.3.zip".to_string(),
