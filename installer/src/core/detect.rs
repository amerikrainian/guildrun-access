...
pub fn game_candidates() -> Vec<GameInstall> {
    ...
    // GUILDRUN_DIR is the same override the mod's own build uses for the game dir.
    if let Ok(value) = std::env::var("GUILDRUN_DIR") {
        add_candidate(
            &mut result,
            &mut seen,
            PathBuf::from(value),
...
mod tests {
    ...
    fn write_markers(dir: &Path) {
        fs::write(dir.join("Guildrun.exe"), "").unwrap();
        let metadata = dir.join("Guildrun_Data").join("il2cpp_data").join("Metadata");
        fs::create_dir_all(&metadata).unwrap();
        fs::write(metadata.join("global-metadata.dat"), "").unwrap();
    }
    ...
    fn rejects_dir_without_game_assembly() {
        let dir = tempfile::tempdir().unwrap();
        fs::write(dir.path().join("Guildrun.exe"), "").unwrap();
        assert!(!validate_game_dir(dir.path()));
        assert_eq!(
