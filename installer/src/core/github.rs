...
pub fn fetch_releases() -> Result<Vec<ReleaseInfo>, String> {
    // GUILDRUN_ACCESS_INSTALLER_RELEASES_URL overrides the feed so an end-to-end
    // test can point at a locally served release.
    let url = std::env::var("GUILDRUN_ACCESS_INSTALLER_RELEASES_URL")
        .unwrap_or_else(|_| GITHUB_RELEASES_URL.to_string());
    let client = Client::builder()
        .user_agent("GuildrunAccessInstaller")
...
mod tests {
    ...
    fn parses_mod_zip_version() {
        assert_eq!(
            parse_mod_zip_version("GuildrunAccess-v1.0.0.zip").as_deref(),
            Some("1.0.0")
        );
        assert!(parse_mod_zip_version("source.zip").is_none());
        assert!(parse_mod_zip_version("GuildrunAccessInstaller.exe").is_none());
    }
