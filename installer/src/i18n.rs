...
//! GUILDRUN_ACCESS_INSTALLER_LANG (a tag like "de" or "pt-br") overrides it. The mod
...
pub fn get() -> &'static Strings {
    if let Ok(tag) = std::env::var("GUILDRUN_ACCESS_INSTALLER_LANG") {
        return pick(&tag);
    }
...
pub static EN: Strings = Strings {
    app_title: "Guildrun Access Installer",
    cli_header: "=== Guildrun Access Installer ===",
    ...
    log_no_asset: "No mod release zip (GuildrunAccess-vX.Y.Z.zip) was found on the latest release.",
    ...
    dir_dialog_title: "Select the Guildrun game directory",
    err_no_zip: "No downloadable release zip was found.",
    err_select_valid: "Select a valid Guildrun game directory.",
    err_close_game_install: "Close Guildrun before installing.",
    err_close_game_uninstall: "Close Guildrun before uninstalling.",
    err_uninstall_managed_only: "Uninstall is only available for installs managed by this installer.",
    confirm_uninstall: "Remove Guildrun Access from this game directory?",
    ...
    msg_first_launch_note: "Launch the game through Steam. The first launch after installing generates the loader's interop assemblies and can take a few minutes; the mod starts speaking at the main menu.",
    msg_install_failed: "Install failed:\n{error}",
    cli_no_valid_install: "No valid Guildrun install selected.",
    ...
    cli_confirm_uninstall: "Remove Guildrun Access? (y/N): ",
