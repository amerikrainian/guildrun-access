...
namespace GuildrunAccess.Core.Buffers
{
    ...
    /// buffers; the current buffer may become empty under the cursor (a focus change changed the
    /// sources' subject), in which case navigation re-resolves to the first non-empty one.
    /// Ported from Harkest Dungeon (dd2a11y).
    /// </summary>
    public sealed class BufferManager
    {
        ...
        public Buffer Get(string key)
        {
            for (int i = 0; i < _buffers.Count; i++)
                if (_buffers[i].Key == key) return _buffers[i];
            return null;
        }
        ...
        public Buffer Current
        {
            get
            {
                ...
                if (!_buffers[_position].IsEmpty) return _buffers[_position];
                for (int i = 0; i < _buffers.Count; i++)
                {
        ...
        /// <summary>Make a buffer current (a focus change re-homing review to the control's own
        /// buffer). Applies <see cref="Buffer.FollowLatest"/>.</summary>
        public void SetCurrent(string key)
        {
            for (int i = 0; i < _buffers.Count; i++)
            {
                if (_buffers[i].Key != key) continue;
                _position = i;
                if (_buffers[i].FollowLatest) _buffers[i].MoveToEnd();
                return;
            }
        }
        ...
        public bool MoveBuffer(int step)
        {
            ...
            for (int i = 1; i <= _buffers.Count; i++)
            {
                ...
                if (idx == _position) break;
                if (_buffers[idx].IsEmpty) continue;
                _position = idx;
                if (_buffers[idx].FollowLatest) _buffers[idx].MoveToEnd();
                return true;
            }
            return false;
        }
