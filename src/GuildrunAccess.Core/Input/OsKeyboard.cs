...
{
    /// <summary>
    /// Reads the OS keyboard typematic settings (initial delay + repeat rate) so our nav key-repeat feels
    /// like the rest of the user's system. On Windows this queries user32 SystemParametersInfo; anywhere
    /// that is unavailable it falls back to sane defaults. Values are read once and cached.
    /// </summary>
    public static class OsKeyboard
    {
        ...
        /// <summary>Force a re-read of the OS settings.</summary>
        ...
        private static void EnsureLoaded()
        {
            ...
            try
            {
                    ...
                    SystemParametersInfo(SPI_GETKEYBOARDSPEED, 0, ref speed, 0))
                {
                    ...
                    CoreLog.Info(string.Format(
                        "OS key repeat: delay={0:0.###}s, interval={1:0.###}s (raw delay={2}, speed={3})",
                        _initialDelay, _repeatInterval, delay, speed));
                    return;
                }
                CoreLog.Info("OS key repeat query returned false; using defaults.");
            }
            catch (Exception e)
            {
                CoreLog.Info("OS key repeat unavailable, using defaults: " + e.Message);
            }
