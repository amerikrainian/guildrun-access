...
{
    /// <summary>
    /// The graph-native table/document emitter: one Tab-stop of vertically-stacked REGIONS (each a
    /// Ctrl+arrow jump target and a context level, so entering one announces its title once via the path
    /// diff), rows navigated Up/Down with the column preserved, cells Left/Right. Framing rules ride the
    /// graph's own mechanisms:
    ...
    ///    row's metadata as extra parts, so moving down the table reads the whole row;
    ...
    public sealed class GraphSheet
    {
        ...
        // Current region state. Row cells carry their LOGICAL column (sparse rows skip empty cells: they
        // are not landable, but vertical navigation still matches columns by logical number).
        ...
        /// <summary>Start a region: a Ctrl+arrow jump target and a context level ("Relics, table").
        /// <paramref name="columns"/> are the headers for the metadata cells (column 0, the primary, has
        /// none); null/empty = a plain one-column list region.</summary>
        public GraphSheet Region(string label, string[] columns = null, string role = null)
        {
            ...
            {
                _b.PushContext(label, role ?? (columns != null && columns.Length > 0 ? Strings.Strings.RoleTable : null),
                    positions: false);
                _contextOpen = true;
            }
            _columns = columns;
        ...
        /// <paramref name="rowRef"/> is the row's DOMAIN OBJECT and should be passed whenever rows can
        /// appear/vanish/reorder: keys derive from it, so a removed row's focus slides to a genuinely
        /// different identity and the differ announces the landing. The primary additionally carries it
        ...
        public GraphSheet Row(NodeVtable primary, object rowRef, params Func<string>[] cells)
        {
            ...
            if (cells != null)
                ...
                {
                    var v = cells[i];
                    if (v == null) continue; // sparse: an empty logical column is not landable
        ...
        private void CloseRegion()
        {
            ...
            // Rows chain across region boundaries: the last row of a region wires to the first of the
        ...
        private void EmitCell(NodeVtable vt, int col)
        {
            ...
            // row's associated element (the column-0 primary) unless the cell defines its own.
            if (col > 0)
            {
            ...
            // DATA rows join this sheet's landing group (the general row-vanish rule). Static lines
            // (a lead row, trailers, empty messages) stay out.
            ...
            // FollowRowRefs=false keeps the hash-based STRUCTURAL key but drops the tier-1 reference, so
            // when the object MOVES TO ANOTHER SHEET focus stays here on the nearest neighbour.
        ...
        // name the destination ROW when landing off-primary; landings on column 0 stay unlabeled.
        ...
        private static string Blank(string v) => string.IsNullOrWhiteSpace(v) ? Strings.Strings.ValueBlank : v;
    }
}
