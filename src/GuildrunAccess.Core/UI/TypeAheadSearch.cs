...
{
    /// <summary>
    /// Type-ahead search engine, ported (with permission, via wotr-access) from OniAccess. Speech and key
    /// routing live in <see cref="GraphNavigator"/>, which feeds typed characters and arrows in; this
    /// class is pure matching/result state.
    ///
    ...
    /// then space-delimited word-prefix abbreviation ("ga pi" matches "gas pipe"). Within a tier, items
    /// keep their LIST ORDER, and matches in the item's NAME (before the first comma) rank ahead of
    /// matches in its appended metadata. Diacritics are ignored. Typing the same letter repeatedly cycles
    /// ALL of that letter's matches in list order.
    /// </summary>
    public class TypeAheadSearch
    {
        ...
        public void Search(int itemCount, System.Func<int, string> nameByIndex, System.Action<int> announceResult)
        {
            ...
            // Within a tier, entries stay in ITEM order. Merge: name (pre-comma) matches across all
            // tiers before metadata (post-comma) matches.
            _workIndices.Clear();
            _workNames.Clear();
