...
namespace GuildrunAccess.Core.Graph
{
    ...
    /// operation, with focus persisting in an external <see cref="GraphState"/>. Ported from wotr-access,
    /// itself from Tanglebeep (with permission) and Factorio Access's key-graph.lua. Two invariants carry:
    ///
    /// <para><b>Down-right total order</b> (<see cref="ComputeOrder"/>): from the start node, go right
    /// until stuck, queueing each down; visits a planar UI in reading order. Nodes down-right cannot reach
    ...
    public sealed class KeyGraph
    {
        ...
        public static void Reconcile(GraphRender render, GraphState state)
        {
            ...
            if (old != null)
            {
                ...
                // Tier 3, the GENERAL row-vanish rule: land on the nearest survivor OF THE SAME LANDING
                // GROUP (this list's rows), next-first then previous, never a header, a lead line or
                // another panel. Only nodes stamped with a LandGroup participate.
                ...
                // row cells, so when a whole row vanished the walk lands on the previous row's LAST cell,
                // a different column. Slide along that row to the column focus was on.
        ...
        // through a header row or another stop does not forget the working column).
        ...
        /// <paramref name="start"/> unchanged when either side is not tabular (Column &lt; 0) or the
        /// column already matches. Horizontal edges never leave a row, so the walk cannot escape it.</summary>
        ...
        /// False when it is not in the render.</summary>
        ...
        /// position, else the SELECTED member (a radio/tab/list item currently checked), else the stop's
        /// first node.</summary>
        ...
        public TreeResult TreeRight()
        {
            ...
            if (node.Expandable && !node.Expanded)
            {
                ...
                {
                    // A lazy drill-in that resolved to nothing: do not leave a silent empty-expanded node.
        ...
        // Change a group's expansion: through its vtable override when declared, else the persistent set.
