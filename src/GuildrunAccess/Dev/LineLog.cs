...
namespace GuildrunAccess.Dev
{
    ...
    /// primitive: one instance holds what the mod has spoken (the driver cannot hear TTS), another the
    /// mod's log lines. Callers poll with a cursor to get only what is new; <see cref="WaitForNew"/>
    /// long-polls so a driver can block until a line lands instead of sleep-looping.
    /// </summary>
    internal sealed class LineLog
    {
        ...
        /// "index: text". <paramref name="next"/> returns the cursor to pass next time.</summary>
        ...
        /// <summary>Block until a line with index &gt;= <paramref name="since"/> exists or the timeout
        /// passes. Returns whether a line arrived.</summary>
