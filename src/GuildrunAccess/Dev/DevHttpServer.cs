...
{
    /// <summary>
    /// Minimal loopback HTTP/1.1 server on a raw TcpListener. Just enough of the protocol for curl
    /// (request line, Content-Length body, one response, connection close), deliberately avoiding
    ...
    internal sealed class DevHttpServer
    {
        ...
        public void Start()
        {
            ...
            _listener.Start();
            _running = true;
            _thread = new Thread(Loop) { IsBackground = true, Name = "GuildrunAccessDevHttp" };
            _thread.Start();
        }
